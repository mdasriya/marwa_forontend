        ...
        <div className='tankMainDiv shadow-lg p-3 mb-5 bg-body-tertiary rounded  bigFontWeight'>   
            ...
            <div>
                ...
                <table class="table" style={{width:'900px'}}>
                    ...
                    <tbody>
                        <tr>  
                        <td><input type="month" class="form-control editableInput bigFontWeight" placeholder="Month" onChange={(e) => setMonth(e.target.value)} /></td>  
                            <td scope="row">
                                <select  class="form-select editableInput bigFontWeight" aria-label="Default select example" value={bankName}  onChange={(e) =>{ setBankName(e.target.value); getTotalAmoun1(e.target.value); setSelectedValues(e.target.value);}}>
                            ...
                            <td><input type="text" class="form-control bigFontWeight" value={accountNo} placeholder="Account No" onChange={(e) => setAccountNo(e.target.value)} disabled/></td>
                ...
                <table class="table" style={{width:'1100px'}}>
                    ...
                    {Staff.map((res,index)=>
                                <tr className='hovereffect' key={index}>
                                    ...
                                    <td id={"sname"+res.staff_id}>{res.first_name +" "+res.last_name}</td>
                                    <td><input type="text" id={"chno"+res.staff_id} class="form-control editableInput bigFontWeight" placeholder="Cheque No."  style={{width:'250px'}}    /></td>
                                    <td><input type="text" id={"amt"+res.staff_id} class="form-control editableInput bigFontWeight" placeholder="Amount"  onChange={(e) => {
        const updatedAmounts = { ...amounts };
        updatedAmounts[res.staff_id] = parseFloat(e.target.value) || 0;
        setAmounts(updatedAmounts);
    }}   style={{width:'250px'}}    /></td>
                                   <td><input type="text" id={"nar"+res.staff_id} class="form-control editableInput bigFontWeight" placeholder="Narration" onChange={(e)=> {setNarration(e.target.value)}} style={{width:'250px'}}    /></td>
                                    <td style={{width:'50px'}}>
                                        <button type="button" style={{height:'30px', paddingTop:'2px'}} id={"data"+res.staff_id} class="btn btn-success" onClick={() => onAdd(res.staff_id)}>Save</button>
                                    </td>
                                </tr>
                            )}
                    </tbody>
                </table>        
                <div style={{marginLeft:'490PX'}}>Total: {getTotalAmount1()}</div>
