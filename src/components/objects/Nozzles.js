...
export default function Nozzles({dbpath1}) {
    ...
    const onAdd = () =>{
        if (nozzle_name.length === 0) {
            ...
            axios.post(url, fData)
              .then(response => { updateSaleFuels(); updateSaleFuelsYesterday(); alert(response.data);  window.location.reload(); })
              .catch(error => {
    ...
    const updateSaleFuels = async () => {
        let query1="INSERT INTO `sale_fuels` (`id`, `date`, `product_name`, `nozzle`, `opening`, `closing`, `sale`, `testing`, `asale`, `rate`, `amount`) VALUES (NULL, '"+datecache+"', '"+product+"', '"+nozzle_name+"', '"+op_meter_reading+"', '', '', '', '', '', '');";
                //alert(query1); 
    ...
    const updateSaleFuelsYesterday = async () => {
        let query1="INSERT INTO `sale_fuels` (`id`, `date`, `product_name`, `nozzle`, `opening`, `closing`, `sale`, `testing`, `asale`, `rate`, `amount`) VALUES (NULL, '"+calcDays()+"', '"+product+"', '"+nozzle_name+"', '', '"+op_meter_reading+"', '', '', '', '', '');";
                //alert(query1); 
        const url1 = dbpath1+'delTank.php';
        let fData1 = new FormData();
        fData1.append('query', query1);  
            ...
            <div>
                ...
                <table class="table">
                    ...
                    <tbody>
                        <tr>    
                            <td scope="row">
                                <input type="text" class="form-control editableInput bigFontWeight" placeholder="Nozzle Name" onChange={(e) => setNozzle_name(e.target.value)} />
                            </td>
                            <td>
                            
                                <select class="form-select editableInput bigFontWeights" aria-label="Default select example" value={product} /* onChange={displaySelectedProduct(product)} */ onChange={(e) => setProduct(e.target.value)}>
                            ...
                            <td>
                                <select class="form-select editableInput bigFontWeights" aria-label="Default select example" value={machine} onChange={e => {
                                ...
                                <option selected>- Machine -</option>
                                    {machinen.map((rest) => (
                                        <option value={rest.dispensing_unit_no}>{rest.dispensing_unit_no}</option>
                                    ))}
                                </select>
                            </td>
                            <td><input type="text" class="form-control editableInput bigFontWeight" placeholder="Side" onChange={(e) => setSide(e.target.value)} /></td>
                            <td><input type="text" class="form-control editableInput bigFontWeight" placeholder="Nozzle no" onChange={(e) => setNozzle_no(e.target.value)} /></td>
                            <td><input type="text" class="form-control editableInput bigFontWeight" placeholder="OP. Meter Reading" onChange={(e) => setOp_meter_reading(e.target.value)} /></td>
                            ...
                            {nozzles.map((res,index)=>
                                <tr className='hovereffect' key={index}>
                                        ...
                                        <td style={{width:'250px'}}>
                                            <button type="button" id={"tank"+res.nozzle_id} class="btn btn-danger" onClick={() => onDelete(res.nozzle_id)}>Delete</button>
                                        </td>
                                        {/*<td style={{width:'50px'}}>
