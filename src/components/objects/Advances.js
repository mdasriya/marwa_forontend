...
export default function Advances({dbpath1}) {
 ...
 /*    const [tgiven, setTgiven] = useState('');
    ...
    const getBalance = async (value) => {
      let query="SELECT balance FROM rwt_advances_transaction WHERE client_name = '"+value+"' ORDER BY transaction_id desc LIMIT 1 ;";
    ...
    const onAdd = () =>{
         if (partyname.length === 0) {
            ...
            let query="INSERT INTO `rwt_advances_transaction` (`transaction_id`, `client_name`, `amount`, `balance`, `narration`, `date`) VALUES (NULL, '"+partyname+"', '"+amount+"', '"+updatedBalance+"', '"+narration+"', '"+date+"');";
            /*  alert(query); */
    ...
    const loadHistory = async (value) => {
      let query="SELECT * FROM rwt_advances_transaction WHERE client_name = '"+value+"';";
              ...
              <h4> <span style={{marginLeft:'800px', marginTop:'10px'}}>Balance : </span> {balance}</h4>
                ...
                <table class="table">
                    <thead>
                        ...
                        <th className='tablebg' style={{width:'355px'}}>Party Name</th>
                       {/*  <th className='tablebg'>Date</th> */}
                            
                            <th className='tablebg' style={{width:'355px'}}>Amount</th>
                           
                            <th className='tablebg' style={{width:'355px'}}>Narration</th>
