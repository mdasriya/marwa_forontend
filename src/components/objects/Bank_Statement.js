                    ...
                    <tbody>
                        <tr>    
                           ...
                           {/*  <td><input type="date" class="form-control" placeholder="Date" onChange={(e) => setDate(e.target.value)} /></td> */}
                            <td scope="row">
                                <select  class="form-select editableInput bigFontWeight" aria-label="Default select example" value={bankName}  onChange={(e) =>{ setBankName(e.target.value);  getTotalAmount1(e.target.value);/*  setSelectedValues(e.target.value); */}}>
                                    <option selected>- select -</option>
                                       
                                    {banks.map((rest) => (  
                                        <option value={rest.bank_account_id}>{rest.name} - {rest.account_no}  </option>
                                    ))}
                                </select>  
                            </td>
                            <td><input type="month" class="form-control editableInput bigFontWeight" placeholder="instruments" value={date} onChange={(e) =>{ setDate(e.target.value); }} /></td>
                         {/*   <td scope="row">
                            ...
                            <td><button type="button" class="btn btn-primary" onClick={onSearch}>Search</button></td> 
                        </tr>
                        
                    </tbody>
                </table>    
            </div>
            <span id="" style={{fontSize:'20px'}}>Narration : </span><span id="shownarration" style={{fontSize:'20px'}}></span>
            <br></br>
            <div>
                <br></br>
                <table class="table table-container" >
                    ...
                    {Statement.map((res,index)=>
                                <tr className='hovereffect' key={index}>
                                   <td>{index+1}</td>
                                    <td>{convertDateFormat(res.date)}</td>
                                    ...
                                    <td style={{width:'200px',}}><input style={{ border:'1px solid'}} type='text' class="form-control editableInput bigFontWeight" id={"status"+res.statemnt_id}   value={inarrration[res.statemnt_id]}  onClick={(e)=>{showNarration(e.target.value)}}   onChange={(e) =>{ showNarration(e.target.value); setInarration({...inarrration, [res.statemnt_id]:e.target.value}); }}  /> 
                                   
                                   </td>
                                   <td> <input style={{width:'20px', height:'20px', border:'1px solid '}} type="checkbox" id={'check'+res.statemnt_id} value='0' class="form-check-input editableInput bigFontWeight"/> </td>
                                
                                    <td style={{width:'120px'}}>
                                        <button type="button" style={{height:'30px', paddingTop:'2px'}} id={"data"+res.statemnt_id} class="btn btn-success" onClick={() => onSave(res.statemnt_id)}>Save</button> &nbsp;&nbsp; 
                                           </td>
                                </tr>
                            )}
                    </tbody>
